...
This workshop will not focus on the theoretical groundwork of these statistical. Rather, we will familiarize ourselves with the process of running statistical models R and learn to leverage tools from `tidyverse` to check assumptions and visualize the model output.
...
Plot the relationship between `species` and `body_mass_g` using boxplot.

Plot the relationship between `bill_depth_mm` and `body_mass_g` using scatter plot. Color the points by `sex` variable.
...
The subject on missing data is an important field of research in statistics and data science, and going in depth about exploring missingness and different methods for dealing with them warrants another workshop. Here, we will briefly go over some important notions, and try out some packages specifically made for visualizing missing data.

There are largely 3 mechanisms in which data can be missing in an increasing order of potential threat to your analysis.
...
There are many great options for visualizing missingness pattern. You can choose to explore yourself using `ggplot2`. There are many packages available [online](https://cran.r-project.org/web/views/MissingData.html#exploration) for this purpose.

In the penguins dataset, most of the missing data were in the sex of the penguins. While looking at different relationships, I found something interesting:
...
Plot the relationship between bill depth and flipper length. Also, try coloring points by different grouping variables.
...
ggplot(penguins) + geom_point(aes(x = bill_depth_mm, y = body_mass_g, color = sex))
```
